<div align="center">
  ...
  **Bộ gõ tiếng Việt nhỏ gọn, đa nền tảng, viết bằng C++17.**
  ...
  Telex · VNI · Unicode dựng sẵn · Windows · macOS · Linux (IBus)
...
VietKi là một bộ gõ tiếng Việt cá nhân. Một engine C++ dùng chung xử lý tiếng
Việt, còn mỗi hệ điều hành có một lớp tích hợp mỏng riêng: Windows/macOS dùng
**keyboard hook + key injection**, còn Linux dùng **engine IBus** (mô hình
composition/preedit) để chạy được cả trên X11 lẫn Wayland.

> [!IMPORTANT]
> Dự án đang trong giai đoạn phát triển. Windows là nền tảng chính; macOS và
> Linux có shell riêng nhưng có thể còn khác biệt về trải nghiệm.
...
Script tạo binary `build-linux/ibus-engine-vietki` và gói Debian
`build-linux/ibus-vietki_<version>_<arch>.deb`:

```sh
sudo apt install ./build-linux/ibus-vietki_<version>_<arch>.deb
ibus restart          # hoặc logout/login
```

Sau khi cài, mở **Settings → Keyboard → Input Sources**, thêm **VietKi**, rồi
chuyển Việt/Anh bằng phím tắt hệ thống (mặc định **Super+Space**). Kiểu gõ
(Telex/VNI) và bật/tắt tiếng Việt có thể đổi nhanh trong menu của IBus ở panel.

Cấu hình Linux nằm tại `~/.config/vietki/config.ini`, mục `[General]`
(`method`, `tone`, `spellCheck`). Các tính năng loại-trừ-theo-app và phím tắt
override của bản Windows/macOS **không áp dụng** trên Linux (mục 7 của
`docs/PHASE4.1.md`). Nếu app Qt/Electron/Java không nhận tiếng Việt, đặt các
biến môi trường `GTK_IM_MODULE=ibus`, `QT_IM_MODULE=ibus`, `XMODIFIERS=@im=ibus`.
...
có thể [mời tác giả một ly cà phê](https://github.com/sponsors/spyhunter88) để
